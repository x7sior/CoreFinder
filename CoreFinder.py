...
class CoreFinder:
    ...
    {1: 4.0, 2: 16.0, 3: 30.0}
    >>> cf.check_excess()
    {'in_core': True, 'excess': {1: -4.0, 2: -16.0, 4: -10.0, 3: -30.0, 5: -14.0, 6: -16.0}}
    >>> cf.check_excess(get_excess=True)
    {'in_core': True, 'excess': {frozenset({1}): -4.0, frozenset({2}): -16.0, frozenset({3}): -30.0, frozenset({1, 2}): -10.0, frozenset({1, 3}): -14.0, frozenset({2, 3}): -16.0}}
    ...
    def _add_v(self, v: dict):
        ...
        if not isinstance(v, dict):
            raise TypeError("Функция выигрыша должна быть словарем")
        ...
        try:
            ...
            raise KeyError(f"Игрок {e} отсутствует в множестве игроков") from e
    ...
    def get_imputation(self, extra_imputation_form: Sequence | None = None) -> dict:
        """
        Вычисляет решение.
    ...
    def check_excess(self,
        ...
        if get_excess or print_excesses:
            ...
            excess_set = {
                frozenset(players_dict[i] for i in get_players(coalition)): exc
                for coalition, exc in excess.items()
            }

            if print_excesses:
                for coalition, value in excess_set.items():
                    print(f"{list(coalition)}: {value}")
    ...
    def _get_players(mask):
        ...
        while mask:
            lsb = mask & -mask
            players.append(lsb.bit_length() - 1)
            mask ^= lsb
        return players
...
class CoreFinderOpt(CoreFinder):
    ...
    def _grand_coalition_x(self):
        ...
        full_sum_x = sum(x)  # сумма всех игроков
        sum_x = {
            (1 << i) ^ main_coalition: full_sum_x - x[i]
            for i in range(n)
        }  # сохраняем суммы игроков для коалиций размера n-1
    ...
    def _run(self):
        ...
        def exc_subcoalitions(k_plus, exc_coalitions, fixed_coalition, players_keys):
            ...
            coalitions_keys = set()  # маски неустойчивых коалиций
            zero_excess_keys = set()  # индексы игроков, без которых зафиксированная коалиция становится устойчивой

            for i in range(k_plus):
                mask = fixed_coalition ^ (1 << players_keys[i])  # маска коалиции без i-го игрока
                if mask in exc_coalitions:
                    coalitions.append(exc_coalitions[mask])
                    coalitions_keys.add(mask)
                else:
                    zero_excess_keys.add(i)
        ...
        def find_main_coalition(k_plus, exc_coalitions, exc_players_keys):
            ...
            for player in exc_players_keys:
                if most_exc_coalition_mask & (1 << player) == 0:  # если игрок не входит в самую недовольную коалицию
                    ...
                    return (
                        exc_subcoalitions(k_plus, exc_coalitions, changed_coalition, changed_keys) + (changed_keys,)
                    )
        ...
        eps = self.eps
        n = self.n
        x, old_sum_x = self._grand_coalition_x()  # старые суммы игроков (коалиций размера k+1)
        for k in range(n - 2, 1, -1):
            ...
            new_sum_x = {}  # новые суммы игроков (коалиций размера k)
            exc_coalitions = {}  # эксцессы неустойчивых коалиций (размера k)
            ...
            while exc_coalitions:
                ...
                # более одной неустойчивой коалиции
                    ...
                    for player, player_value in extra_x.items():
                        del_excesses = []

                        for coalition, value in exc_coalitions.items():
                            if coalition & (1 << player):
                                excesses_value = value - player_value

                                if excesses_value > eps:
                                    exc_coalitions[coalition] = excesses_value
                                else:
                                    del_excesses.append(coalition)
                    ...
                    for coalition, excess in exc_coalitions.items():
                        for player in get_players(coalition):
                            exc_players[player] += excess

                # пересчитываем суммы добавочных выплат каждого игрока
                for player, value in extra_x.items():
                    total_extra_x[player] += value

            # пересчитываем суммы игроков
            for player, value in total_extra_x.items():
                if value > eps:  # если сумма добавочных выплат игрока больше нуля
                    for coalition in new_sum_x:  # среди всех сумм
                        if coalition & (1 << player):  # находим коалиции, в которые входит игрок
                            new_sum_x[coalition] += value  # и добавляем сумму добавочных выплат
            old_sum_x = new_sum_x
...
class CoreFinderAlt(CoreFinder):
    ...
    def _run(self):
        ...
        v = self.v
        v1 = v[1]
        vn = next(iter(v[n].values()))

        # стартовые значения
        x = {i: v1[1 << i] if v1[1 << i] > 0 else 0 for i in range(n)}
        x_min = x
        best_full_sum_x = float("inf")
        ...
        while exc_coalitions:
            ...
            for k in exc_coalitions:
                ...
                for coalition in vk:
                    ...
                    # помечаем лишние коалиции из списка эксцессов
                    if vk[coalition] <= eps:
                        del_exc_coalitions.append(coalition)

                # удаляем помеченные коалиции
                for coalition in del_exc_coalitions:
                    del vk[coalition]
                    amount -= 1

                # помечаем лишние группы коалиций
                if amount == 0:
                    del_k.append(k)

            # удаляем помеченные группы
            for k in del_k:
            ...
            full_sum_x = sum(tmp_x.values())

            if full_sum_x <= best_full_sum_x:
                self.x = tmp_x
                best_full_sum_x = full_sum_x

                if early_stop and best_full_sum_x <= vn:
                    break
            ...
            for main_coalition in vk:
                ...
                for i in range(k):
                    ...
                    if coalition_mask in vkm:
                        coalitions.append(vkm[coalition_mask])
                    else:
                        zero_excess_keys.add(i)
