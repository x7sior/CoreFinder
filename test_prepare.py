...
def pregenerate_games(game_type: str,
    ...
    seeds = [i for i in range(total) if i not in seeds]
...
def compute(game_type: str,
    ...
    try:
        ...
        if os.path.exists(f'{compute_path}\\{coefficient}.xlsx'):
            raise Exception(f'Повреждён файл: {compute_path}\\{coefficient}.xlsx') from e
    ...
    for seed in tqdm(game_seeds):
        ...
        shapley_flag = False
        if os.path.exists(f'{precompute_path}\\{seed}\\Shapley.xlsx') and seed not in table['shapley_dt']:
            shapley_flag = True

        cf_flag = False
        if os.path.exists(f'{precompute_path}\\{seed}\\CF.xlsx') and seed not in table['cf_dt']:
            cf_flag = True

        cfo_flag = False
        if os.path.exists(f'{precompute_path}\\{seed}\\CFO.xlsx') and seed not in table['cfo_dt']:
            cfo_flag = True

        cfa_flag = False
        if os.path.exists(f'{precompute_path}\\{seed}\\CFA.xlsx') and seed not in table['cfa_dt']:
            cfa_flag = True
...
def compute_distance(game_type: str,
    ...
    for seed in tqdm(game_seeds):
        ...
        if nucleolus_flag:
            ...
            if os.path.exists(f'{precompute_path}\\{seed}\\Shapley.xlsx') and seed not in table['nuc_vs_shapley']:
                nuc_vs_shapley = True

            nuc_vs_cf = False
            if os.path.exists(f'{precompute_path}\\{seed}\\CF.xlsx') and seed not in table['nuc_vs_cf']:
                nuc_vs_cf = True

            nuc_vs_cfo = False
            if os.path.exists(f'{precompute_path}\\{seed}\\CFO.xlsx') and seed not in table['nuc_vs_cfo']:
                nuc_vs_cfo = True

            nuc_vs_cfa = False
            if os.path.exists(f'{precompute_path}\\{seed}\\CFA.xlsx') and seed not in table['nuc_vs_cfa']:
                nuc_vs_cfa = True

        if os.path.exists(f'{precompute_path}\\{seed}\\Shapley.xlsx'):

            shapley_vs_cf = False
            if os.path.exists(f'{precompute_path}\\{seed}\\CF.xlsx') and seed not in table['shapley_vs_cf']:
                shapley_vs_cf = True

            shapley_vs_cfo = False
            if os.path.exists(f'{precompute_path}\\{seed}\\CFO.xlsx') and seed not in table['shapley_vs_cfo']:
                shapley_vs_cfo = True

            shapley_vs_cfa = False
            if os.path.exists(f'{precompute_path}\\{seed}\\CFA.xlsx') and seed not in table['shapley_vs_cfa']:
                shapley_vs_cfa = True
